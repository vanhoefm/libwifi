...
D-Bus is a generic interface to perform remote method calls and to send and observe signals. Unlike the above custom control interface provided by hostap, D-Bus is _not_ string-based communication. The D-Bus commands that `wpa_supplicant` supports can be found in [wpa_supplicant/dbus/dbus_new.c](https://w1.fi/cgit/hostap/tree/wpa_supplicant/dbus/dbus_new.c?h=hostap_2_11#n3300). To better understand these commands, it's useful to monitor them while using Wi-Fi on Linux.

For background, D-Bus provides a system bus for messages related to the system as a whole, e.g., hardware connects and disconnects, and a session bus for messages related to a specific user session, e.g. an X or Wayland session. The `wpa_supplicant` daemon is controlled over the system bus. By default, it's possible to inspect the events emitted by a program using the following commands:

```
# This provides detailed output, quickly fills up the screen
sudo dbus-monitor --system "sender=fi.w1.wpa_supplicant1"
# This provides more compressed output
sudo gdbus monitor --system --dest fi.w1.wpa_supplicant1
```

However, methods calls to `wpa_supplicant` will not be show. To see those, you must first create the file [`/etc/dbus-1/system-local.conf`](https://wiki.ubuntu.com/DebuggingDBus) with the following content:
...
After creating the file, reboot your system. You can then execute the following to see both commands send to `wpa_supplicant` and events generated by `wpa_supplicant`:

```
# You can optionally add the --profile parameter to have a shorter output with less info
sudo dbus-monitor --system "destination=fi.w1.wpa_supplicant1" "sender=fi.w1.wpa_supplicant1"
```

As an example, NetworkManager performs methods calls such as `"AddNetwork"` in the file [src/core/supplicant/nm-supplicant-interface.c](https://gitlab.freedesktop.org/NetworkManager/NetworkManager/-/blob/23ffa5fc6e7acbd7a96138c6c18f478f5127177d/src/core/supplicant/nm-supplicant-interface.c#L2394). Additionally, NetworkManager seems to monitor for events in the function [`_properties_changed_main` in the same file](https://gitlab.freedesktop.org/NetworkManager/NetworkManager/-/blob/23ffa5fc6e7acbd7a96138c6c18f478f5127177d/src/core/supplicant/nm-supplicant-interface.c#L1932).
